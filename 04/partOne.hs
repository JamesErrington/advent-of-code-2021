...
bingo :: [Int] -> [Board] -> Either Int [Board]
bingo [] boards = Right boards
bingo (n : ns) boards = case result of
  Left winningBoard -> Left $ n * computeScore winningBoard
  Right newBoards -> bingo ns newBoards
  where
    result = mapM (checkBoard . markBoard n) boards
